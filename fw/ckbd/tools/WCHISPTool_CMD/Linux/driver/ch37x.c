...
/********************************************************************************************/
static ssize_t ch37x_bulk_read(struct usb_ch37x *dev, u8 num, void *buf,
			       u32 count)
{
	int rv;
	int bytes_read = 0;
	...
	rv = usb_bulk_msg(dev->udev,
			  usb_rcvbulkpipe(dev->udev,
					  dev->bulk_in_endpointAddr[num]),
			  buffer, bytes_to_read, &bytes_read, dev->readtimeout);
	if (rv == 0) {
...
static ssize_t ch37x_bulk_write(struct usb_ch37x *dev, int num, void *buf,
				unsigned long count)
{
	int rv;
	int bytes_write = 0;
	...
	rv = usb_bulk_msg(
		...
		buffer, bytes_to_write, &bytes_write, dev->writetimeout);
	if (rv == 0) {
		rv = bytes_write;
	}
...
long ch37x_ioctl(struct file *ch37x_file, unsigned int cmd, unsigned long arg)
{
	...
	case GetVersion:
		...
		break;
	case GetDeviceID:
		retval = copy_to_user((__u16 __user *)arg, dev->ch37x_id,
				      sizeof(__u16) * 2);
		break;
	case GetDeviceDes:
		retval = usb_get_descriptor(dev->udev, USB_DT_DEVICE, 0x00,
					    &dev->udev->descriptor,
					    sizeof(dev->udev->descriptor));
		if (retval)
			retval = copy_to_user((u8 __user *)arg,
					      &dev->udev->descriptor,
					      sizeof(dev->udev->descriptor));
		break;
	...
	case CH37X_BULK_READ:
		...
		retval = ch37x_bulk_read(dev, epnum, (char __user *)arg1,
					 bytes_read);
		break;
	case CH37X_BULK_WRITE:
		...
		arg1 = (unsigned long)((u8 *)arg + 5);
		retval = ch37x_bulk_write(dev, epnum, (char __user *)arg1,
					  bytes_write);
		break;
...
static int ch37x_probe(struct usb_interface *interface,
		       const struct usb_device_id *id)
{
	...
	for (i = 0; i < iface_desc->desc.bNumEndpoints; ++i) {
		...
		if ((endpoint->bEndpointAddress & 0x80) &&
			...
			dev->bulk_in_endpointAddr[dev->bulk_in_num] =
		...
		if (!(endpoint->bEndpointAddress & 0x80) &&
		    ((endpoint->bmAttributes & 3) == 0x02)) {
			dev->bulk_out_endpointAddr[dev->bulk_out_num] =
				endpoint->bEndpointAddress;
			dev->bulk_out_num++;
		}
	}
	...
	dev_info(&interface->dev, "USB ch37x device now attached to ch37x-%d",
		 interface->minor);
	return 0;
