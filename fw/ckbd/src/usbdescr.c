...
static INT32 ItemSData(HID_ITEM *itemInfo)
{
	switch (itemInfo->size)
	{
        case 1: return SIGNEX(itemInfo->value.s8, 7); //itemInfo->value.s8;
        case 2: return SIGNEX(itemInfo->value.s16, 15);//itemInfo->value.s16;
                ...
                if (item.tag == HID_MAIN_ITEM_TAG_INPUT) {
                    if (HIDParseState.appUsagePage == REPORT_USAGE_PAGE_GENERIC && (
                        ...
                        if (currHidReport == NULL) {
                            pInterface->Reports = UsbListAdd(pInterface->Reports, sizeof(HID_REPORT), hidGlobalPnt->reportID);
                        ...
                        if (ItemUData(&item) & HID_INPUT_VARIABLE) {
                            ...
                            } else if (HIDParseState.hidLocal.usageMin != 0xFFFF && HIDParseState.hidLocal.usageMax != 0xFFFF && hidGlobalPnt->reportSize == 1) {
                    ...
                    if (currHidReport != NULL) {
                        currHidReport->length = HIDParseState.startBit;
                    }
                }
	...
	while (index < totalLen) {
		...
		if (descrType == USB_DESCR_TYP_INTERF) {
			...
			if (pItfDescr->bInterfaceClass == USB_DEV_CLASS_HID) {
                pUsbDevice->Interfaces = UsbListAdd(pUsbDevice->Interfaces, sizeof(INTERFACE), pItfDescr->bInterfaceNumber);
...
static HID_SEG *currSegPnt = 0;
static uint16_t tempSB = 0;        // running startbit that increments as segments get matched

void CreateSeg(INTERFACE *pInterface)
{
	static HID_REPORT * __xdata rep;
	rep = (HID_REPORT *)UsbListGetData(pInterface->Reports, HIDParseState.hidGlobal.reportID);
	rep->HidSegments = UsbListAdd(rep->HidSegments, sizeof(HID_SEG), 0x69);
